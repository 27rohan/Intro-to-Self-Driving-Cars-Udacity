...
vector< vector<float> > normalize(vector< vector <float> > grid) {
  // todo - your code here
	float total = 0.0;
  	...
  	vector< vector<float> > normalizedGrid;
	normalizedGrid = grid;
...
// Equivalent to a%n in Python
signed int getModulus(signed int a, signed int n) { 
	signed int r = a % n;
	if (r >= 0)
		return r;
	else
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	// your code here
	vector < vector <float> > blurredGrid;
		...
		for (int j = 0; j < cols; j++) {
			...
			for (signed int dx = -1; dx < 2; dx++) {
				for (signed int dy = -1; dy < 2; dy++) {
					...
					new_i = getModulus((i + dy),rows);
					new_j = getModulus((j + dx),cols);
					blurredGrid[new_i][new_j] += mult * grid_val;
				}}}}
	return normalize(blurredGrid);
}
/** -----------------------------------------------
#
#
